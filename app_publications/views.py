...
from app_publications.forms import AddPostMyForm, UpdatePostMyForm
from app_publications.models import Posts
from app_subscriptions.models import Subscriptions
from app_subscriptions.services import create_session, retrieve_session


class PostListView(ListView):
    """Список публичных публикаций"""
    model = Posts
    template_name = 'app_publications/posts_list.html'
    extra_context = {
        'title': 'Публикации'
    }
...
class PostDetailView(DetailView):
    ...
    model = Posts
    template_name = 'app_publications/posts_detail.html'
...
class PostPayRedirectView(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        if subscription.filter(post=post, pay_status='complete'):
            return reverse_lazy("app_publications:posts_detail", kwargs={'pk': post.pk})
        ...
        if post.paid_published:
            success_url = f'http://{get_current_site(self.request)}{reverse_lazy("app_subscriptions:list")}'
        ...
        return reverse_lazy("app_publications:posts")


class PostMyListView(PermissionRequiredMixin, ListView):
    """Список моих публикаций"""
    model = Posts
    template_name = 'app_publications/posts_my_list.html'
    extra_context = {
        'title': 'Мои публикации'
    }
    permission_required = ['app_publications.view_posts']
...
class PostMyDetailView(PermissionRequiredMixin, DetailView):
    """Детали публикаций"""
    model = Posts
    template_name = 'app_publications/posts_my_detail.html'
    permission_required = ['app_publications.view_posts']
...
class PostMyCreateView(PermissionRequiredMixin, CreateView):
    ...
    model = Posts
    template_name = 'app_publications/posts_my_form.html'
    permission_required = ['app_publications.add_posts']
    extra_context = {
    ...
    def get_success_url(self):
        return reverse_lazy('app_publications:my_posts_detail', kwargs={'pk': self.object.pk})
...
class PostMyUpdateView(PermissionRequiredMixin, UpdateView):
    ...
    model = Posts
    template_name = 'app_publications/posts_my_form.html'
    permission_required = ['app_publications.change_posts']
...
class PostMyDeleteView(PermissionRequiredMixin, DeleteView):
    ...
    model = Posts
    template_name = 'app_publications/posts_my_confirm_delete.html'
    permission_required = ['app_publications.delete_posts']
    success_url = reverse_lazy('app_publications:my_posts')
